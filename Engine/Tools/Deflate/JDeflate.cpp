...
JDeflate::DYNAMIC_NODE::DYNAMIC_NODE(){
  Frequency = 0;
  BitLength = 0;
  Next      = 0;
  Parent    = 0;
...
void JDeflate::DefaultCodes(CODE* Literals, CODE* Distances){
  unsigned j, c;
...
void JDeflate::AddBinary(const unsigned char* Buffer, unsigned Length){
  unsigned       j, l;
  unsigned char* Temp;
  ...
  if(l + Length > BufferSize){
    ...
    for(j = 0; j < l; j++){
      Temp[j] = JDeflate::Buffer[j];
    }
    delete[] JDeflate::Buffer;
    JDeflate::Buffer = Temp;
  }

  for(j = 0; j < Length; j++){
    JDeflate::Buffer[l+j] = Buffer[j];
  }

  JDeflate::Length += Length;
}
//------------------------------------------------------------------------------

void JDeflate::AddBits(
  unsigned  Bits,
  unsigned  Count,
...
bool JDeflate::FindLengthDistancePair(
...
unsigned JDeflate::LengthToLiteral(unsigned Length, CODE* ExtraBits){
  ExtraBits->Code     = 0;
...
unsigned JDeflate::EncodeDistance(unsigned Distance, CODE* ExtraBits){
  ExtraBits->Code     = 0;
  ExtraBits->BitCount = 0;
...
void JDeflate::AddToQueue(DYNAMIC_NODE* Node){
  DYNAMIC_NODE* Next;
  DYNAMIC_NODE* Prev = 0;
...
unsigned JDeflate::FindLengths(DYNAMIC_NODE* Tree, unsigned Length){
  unsigned l1, l2;
...
void JDeflate::RebuildQueue(DYNAMIC_NODE* Tree){
...
void JDeflate::BuildCodes(CODE* Codes, SYMBOL* Symbols, unsigned Count){
  SYMBOL    Temp;
  unsigned  j, q, w;
...
void JDeflate::DynamicCodes(
  ...
  if(CodeLengthCodes[15].BitCount == 0){ NCodeLengthCodes--;
  if(CodeLengthCodes[ 1].BitCount == 0){ NCodeLengthCodes--;
  if(CodeLengthCodes[14].BitCount == 0){ NCodeLengthCodes--;
  if(CodeLengthCodes[ 2].BitCount == 0){ NCodeLengthCodes--;
  if(CodeLengthCodes[13].BitCount == 0){ NCodeLengthCodes--;
  if(CodeLengthCodes[ 3].BitCount == 0){ NCodeLengthCodes--;
  if(CodeLengthCodes[12].BitCount == 0){ NCodeLengthCodes--;
  if(CodeLengthCodes[ 4].BitCount == 0){ NCodeLengthCodes--;
  if(CodeLengthCodes[11].BitCount == 0){ NCodeLengthCodes--;
  if(CodeLengthCodes[ 5].BitCount == 0){ NCodeLengthCodes--;
  if(CodeLengthCodes[10].BitCount == 0){ NCodeLengthCodes--;
  if(CodeLengthCodes[ 6].BitCount == 0){ NCodeLengthCodes--;
  if(CodeLengthCodes[ 9].BitCount == 0){ NCodeLengthCodes--;
  if(CodeLengthCodes[ 7].BitCount == 0){ NCodeLengthCodes--;
  if(CodeLengthCodes[ 8].BitCount == 0){ NCodeLengthCodes--;
  }}}}}}}}}}}}}}}
...
unsigned char* JDeflate::Deflate(
  ...
  JDeflate::WindowSize = WindowSize;
  ...
  JDeflate::Buffer     = new unsigned char[1];
  ...
  *Length = JDeflate::Length;
  Buffer = new unsigned char[*Length];
  for(j = 0; j < *Length; j++){
    Buffer[j] = JDeflate::Buffer[j];
  }

  delete[] JDeflate::Buffer;
  JDeflate::BufferSize = 0;
  JDeflate::Length     = 0;
...
unsigned JDeflate::DecodeSymbol(
  HUFFMAN*       Tree,
  unsigned char* Buffer,
  unsigned*      Index,
  unsigned*      BitIndex
...
void JDeflate::DefaultTree(HUFFMAN* LiteralTree, HUFFMAN* DistanceTree){
  unsigned j, c;
...
void JDeflate::BuildTree(HUFFMAN* Root, SYMBOL* Symbols, unsigned Count){
  SYMBOL    Temp;
  unsigned  j, q, w;
  unsigned  MaxBitLength;
  unsigned* BitLengthCount;
...
unsigned JDeflate::LiteralToLength(
  unsigned       Literal,
  unsigned char* Buffer,
  unsigned*      Index,
  unsigned*      BitIndex
...
unsigned JDeflate::DecodeDistance(
  unsigned       Distance,
  unsigned char* Buffer,
  unsigned*      Index,
  unsigned*      BitIndex
      ...
      while(Index < *Length){
        ...
        if(Literal < 0x100){
          ...
          while(Literal){ // Use Literal as Length
            c = JDeflate::Buffer[(int)(JDeflate::Length)-Distance];
            AddBinary(&c, 1);
            Literal--;
          }
        }
