...
void ENGINE::AddText(pdfContents* Contents, pdfPage* Page){
  double  Left,  Bottom;
  ...
  if(HaveHeader){
    ...
    Bottom = ThePageTop + Header.Space;

    Resize(Page,
      Left   + tLeft,
      Bottom + tBottom,
      Left   + tRight,
      Bottom + tTop + Header.Space
    );
  ...
  if(HaveFooter){
    ...
    Left   = Centre - (tLeft+tRight)/2.0 - tLeft;
    Bottom = ThePageBottom - (tTop-tBottom) - Footer.Space;

    Resize(
      Page,
      Left   + tLeft,
      Bottom + tBottom - Footer.Space,
      Left   + tRight,
      Bottom + tTop
    );
...
int ENGINE::Run(const char* FileName, const char* Title){
  ...
  if(!ThePage || NewPage || !Combine){
    if(ThePage) AddText(TheContents, ThePage);
    Page        = new PAGE(Page);
    ThePage     = Page->Page;
    ThePageUsed = false;
  }
